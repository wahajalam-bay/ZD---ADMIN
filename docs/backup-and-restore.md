...
- **AWS S3 / Cloudflare R2**: enable bucket **versioning** so deletions/overwrites are
...
- Object keys are referenced from PostgreSQL — restore the database and the bucket
...
Do a quarterly test restore into a scratch database + bucket, boot the app against
them (`DATABASE_URL`, `S3_*` env), sign in, and spot-check a published week including
photo evidence.
